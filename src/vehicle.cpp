...
{ 
  ...
  this->a = a; 
  this->state = state;
  max_acceleration = .448; //.224; // change in velocity .224 ~ 5m/s^2 (limit 10)
}
...
vector<Vehicle> Vehicle::choose_next_state(vector<Vehicle> &predictions) {
  ...
  for (vector<string>::iterator it = states.begin(); it != states.end(); ++it) {
    ...
    if (trajectory.size() != 0) {
      ...
      Vehicle t0 = trajectory[0];
      std::cout << "init trajectory lane, s, v, a, state " << t0.lane << ", " << t0.s << ", " << t0.v << ", " << t0.a << ", " << t0.state << std::endl;       
       Vehicle t1 = trajectory[0];
      std::cout << "final trajectory lane, s, v, a, state " << t1.lane << ", " << t1.s << ", " << t1.v << ", " << t1.a << ", " << t1.state << std::endl;       
...
vector<double> Vehicle::get_kinematics(vector<Vehicle> &predictions, 
                                      int lane) {
  ...
  new_accel = new_velocity - this->v; // Equation: (v_1 - v_0)/t = acceleration *** time????
...
vector<Vehicle> Vehicle::keep_lane_trajectory(vector<Vehicle> &predictions) {
  ...
  // vector<Vehicle> trajectory = {Vehicle(lane, this->s, this->v, this->a, this->state)};
  ...
  vector<Vehicle> trajectory = {Vehicle(this->lane,this->s,this->v,this->a,this->state), 
                                Vehicle(this->lane,new_s,new_v,new_a,"KL")};  
  return trajectory;
}
                                                ...
                                                vector<Vehicle> &predictions) 
{
  ...
  {
      next_lane_vehicle = predictions[i];//->first[0];
...
bool Vehicle::get_vehicle_behind(vector<Vehicle> &predictions, 
                                 int lane, Vehicle &rVehicle) 
{
  ...
  for (int i = 0; i < predictions.size(); ++i) 
  {
    ...
    if (temp_vehicle.lane == this->lane && temp_vehicle.s < this->s 
        && temp_vehicle.s > max_s && temp_vehicle.s >= this->s - this->preferred_buffer) 
    {
...
bool Vehicle::get_vehicle_ahead(vector<Vehicle> &predictions, 
                                int lane, Vehicle &rVehicle) {
  ...
  for (int i = 0; i < predictions.size(); ++i) 
  {
    temp_vehicle = predictions[i];//->second[0];
    if (temp_vehicle.lane == this->lane && temp_vehicle.s > this->s 
        && temp_vehicle.s < min_s && (temp_vehicle.s - this->s)  <=  this->preferred_buffer) {
